...
def main():
  ...
  def update_robots():
    ...
    key_qpos = None
    root_pos = np.array(robot_cfg.init_state.pos, dtype=float)
    root_quat = np.array(robot_cfg.init_state.rot, dtype=float)
    if robot_cfg.init_state.joint_pos is None and robot_spec.keys:
      key_qpos = np.array(robot_spec.keys[0].qpos, dtype=float)
    ...
    # Apply root pose and joint poses.
    if key_qpos is not None:
      robot_data.qpos[: len(key_qpos)] = key_qpos
      if robot_model.njnt and robot_model.jnt_type[0] == mujoco.mjtJoint.mjJNT_FREE:
        root_pos = key_qpos[:3].copy()
        root_quat = key_qpos[3:7].copy()
        robot_data.qpos[:3] = 0.0
        robot_data.qpos[3:7] = root_quat
    else:
    ...
    batched_wxyzs = root_quat[None].repeat(
