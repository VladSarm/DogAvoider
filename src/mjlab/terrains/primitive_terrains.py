...
@dataclass(kw_only=True)
class BoxRoomWithPillarsTerrainCfg(SubTerrainCfg):
  """Flat room with perimeter walls and random rectangular pillars."""
  ...
  num_pillars: int = 8
  """Number of random pillars to place in the room interior."""

  pillar_size: tuple[float, float] = (0.3, 0.3)
  """Pillar footprint (x, y) in meters."""

  pillar_height: float = 1.0
  """Pillar height in meters."""

  spawn_clearance: float = 1.0
  """Radius of the obstacle-free region around the room center."""

  pillar_margin: float = 0.2
  """Additional margin from walls to pillar edges."""

  min_pillar_spacing: float = 0.15
  """Minimum gap between pillar edges."""
  ...
  def function(
    ...
    del difficulty  # Fixed room layout; difficulty is intentionally ignored.
    body = spec.body("terrain")
    ...
    if self.pillar_height <= 0:
      raise ValueError("pillar_height must be positive")
    if self.pillar_size[0] <= 0 or self.pillar_size[1] <= 0:
      raise ValueError("pillar_size dimensions must be positive")
    if self.num_pillars < 0:
      raise ValueError("num_pillars must be non-negative")

    room_center = np.array([self.size[0] / 2.0, self.size[1] / 2.0, 0.0])
    geometries: list[TerrainGeometry] = []

    floor = make_plane(body, self.size, 0.0, center_zero=False)[0]
    geometries.append(TerrainGeometry(geom=floor, color=(0.5, 0.5, 0.5, 1.0)))

    wall_half_h = self.wall_height / 2.0
    wall_color = brand_ramp(_MUJOCO_BLUE, 0.2)
    wall_specs = (
      (
        (self.size[0] / 2.0, self.size[1] - self.wall_thickness / 2.0, wall_half_h),
        (self.size[0] / 2.0, self.wall_thickness / 2.0, wall_half_h),
      ),
      (
        (self.size[0] / 2.0, self.wall_thickness / 2.0, wall_half_h),
        (self.size[0] / 2.0, self.wall_thickness / 2.0, wall_half_h),
      ...
      (
        (self.wall_thickness / 2.0, self.size[1] / 2.0, wall_half_h),
        (self.wall_thickness / 2.0, self.size[1] / 2.0, wall_half_h),
      ),
      (
        (self.size[0] - self.wall_thickness / 2.0, self.size[1] / 2.0, wall_half_h),
        (self.wall_thickness / 2.0, self.size[1] / 2.0, wall_half_h),
      ),
    )
    for pos, size in wall_specs:
      wall = body.add_geom(type=mujoco.mjtGeom.mjGEOM_BOX, pos=pos, size=size)
      geometries.append(TerrainGeometry(geom=wall, color=wall_color))

    half_x = self.pillar_size[0] / 2.0
    half_y = self.pillar_size[1] / 2.0
    pillar_half_h = self.pillar_height / 2.0
    min_x = self.wall_thickness + self.pillar_margin + half_x
    max_x = self.size[0] - self.wall_thickness - self.pillar_margin - half_x
    min_y = self.wall_thickness + self.pillar_margin + half_y
    max_y = self.size[1] - self.wall_thickness - self.pillar_margin - half_y

    if min_x > max_x or min_y > max_y:
      raise ValueError(
        "Room interior is too small for the requested wall/pillar dimensions."
      )

    placed_centers: list[tuple[float, float]] = []
    required_center_spacing_x = self.pillar_size[0] + self.min_pillar_spacing
    required_center_spacing_y = self.pillar_size[1] + self.min_pillar_spacing
    center_half_diag = 0.5 * np.hypot(self.pillar_size[0], self.pillar_size[1])

    for pillar_idx in range(self.num_pillars):
      placed = False
      for _ in range(self.max_sampling_attempts):
        cx = float(rng.uniform(min_x, max_x))
        cy = float(rng.uniform(min_y, max_y))

        if np.hypot(cx - room_center[0], cy - room_center[1]) < (
          self.spawn_clearance + center_half_diag
        ):
          continue
        ...
        for px, py in placed_centers:
          if (
            abs(cx - px) < required_center_spacing_x
            and abs(cy - py) < required_center_spacing_y
          ):
            overlaps = True
            break
        if overlaps:
        ...
        pillar = body.add_geom(
          type=mujoco.mjtGeom.mjGEOM_BOX,
          pos=(cx, cy, pillar_half_h),
          size=(half_x, half_y, pillar_half_h),
        )
        pillar_color = brand_ramp(_MUJOCO_RED, pillar_idx / max(self.num_pillars, 1))
        geometries.append(TerrainGeometry(geom=pillar, color=pillar_color))
      ...
      if not placed:
        raise ValueError(
          "Failed to place all pillars without overlap. "
          "Reduce num_pillars, spawn_clearance, or margins."
        )

    return TerrainOutput(origin=room_center, geometries=geometries)
